...
import com.goormthom.danpoong.reboot.dto.response.*;
...
public interface AnalysisDocs {
    ...
    @ApiResponses(value = {
            @ApiResponse(
                    ...
                    description = """
                            - 캘린더를 상세 조회 성공.
                            - 해당 일자의 업무들의 정보가 포함된 응답 데이터 반환.
                            """,
                    content = @Content(
                            mediaType = "application/json",
                            schema = @Schema(implementation = CalendarDetailResponseDto.class),
                            array = @ArraySchema(schema = @Schema(implementation = CalendarDetailResponseDto.class))
                    )
            ),
            @ApiResponse(
                    responseCode = "401",
    ...
    ResponseDto<?> getCalendarDetail(@Parameter(hidden = true) @UserId UUID userId, @RequestParam(name = "date") LocalDate date);
}
